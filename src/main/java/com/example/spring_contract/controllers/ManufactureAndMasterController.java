...
public class ManufactureAndMasterController {
    ...
    @GetMapping("/manufacture/{id}/change")
    public String changeManufacture(Model model, @PathVariable int id) {
        Manufacture manufacture = manufactureRepository.findById(id).orElseThrow();
        model.addAttribute("man", manufacture);
        model.addAttribute("mas", masterRepository.findMasterWithoutManufacture());
        return "product/changeManufacture";
    }
    @PostMapping("/manufacture/{id}/change")
    public String postChangeManufacture(Model model, @PathVariable int id,@RequestParam Master masters) {
        Manufacture manufacture = manufactureRepository.findById(id).orElseThrow();
        manufacture.setMaster(masters);
    ...
    @GetMapping("/manufacture/master/{id}/change")
    public String changeMaster(Model model, @PathVariable int id) {
        Master master = masterRepository.findById(id).orElseThrow();
        model.addAttribute("master", master);
        return "product/changeMaster";
    }

    @PostMapping("/manufacture/master/{id}/change")
    public String postChangeMaster(Model model, @PathVariable int id, @RequestParam String name, @RequestParam Optional<Integer> salary, @RequestParam String spec) {
        Master master = masterRepository.findById(id).orElseThrow();
        master.setName(name);
        master.setSalary(salary.orElseThrow());
        master.setSpecializations(spec);
